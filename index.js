...
app.get('/redirect', (req, res) => {
  if (lastRedirectUrl) {
    ...
    res.status(404).json({ error: 'No redirect available yet.' });
  }
});
...
;(async () => {
  ...
  const KEY_ID = "b90a69f0-773a-4e34-b2e6-b123b4b12899"

  // Make sure the wallet addresses starts with https:// (not $)
  const CLIENT_WALLET_ADDRESS_URL =  "https://ilp.interledger-test.dev/ahorro"
  const SENDING_WALLET_ADDRESS_URL =  "https://ilp.interledger-test.dev/angel"
  const RECEIVING_WALLET_ADDRESS_URL = "https://ilp.interledger-test.dev/hiiii"
  ...
  console.log( receivingWalletAddress.publicName)
  ...
  lastRedirectUrl = outgoingPaymentGrant.interact.redirect;
